...
public class TankService
{
    ...
    private static void draw(Graphics g, Tank tank)
    {
        ...
        // 画血条
        c = g.getColor();
        g.setColor(Constants.TANK_BOOLD_COLOR);
        g.drawRect(x1, y1 - Constants.TANK_BOOLD_HEIGHT, tank.getWidth(),
                Constants.TANK_BOOLD_HEIGHT);
        int w = tank.getWidth() * tank.getNowLife() / tank.getLife();
        g.fillRect(x1, y1 - Constants.TANK_BOOLD_HEIGHT, w,
                Constants.TANK_BOOLD_HEIGHT);
        g.setColor(c);
    }
