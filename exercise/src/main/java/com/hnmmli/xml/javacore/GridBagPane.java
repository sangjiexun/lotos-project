...
public class GridBagPane extends JPanel
{
    ...
    public GridBagPane(String filename)
    {
        ...
        try
        {
            ...
            if (filename.contains("-schema"))
            {
                int count = this.removeElementContentWhitespace(doc.getDocumentElement());
                System.out.println(count + " whitespace nodes removed.");
            }

            this.parseGridbag(doc.getDocumentElement());
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    private int removeElementContentWhitespace(Element e)
    {
        ...
        for (int i = 0; i < children.getLength() && allTextChildrenAreWhiteSpace; i++)
        {
            ...
            if (child instanceof Text && ((Text) child).getData().trim().length() > 0)
            {
                ...
                {
                    elements++;
                    count += this.removeElementContentWhitespace((Element) child);
                }
        }
    ...
    private void parseCell(Element e, int r, int c)
    {
        ...
        {
            this.constraints.gridx = Integer.parseInt(value);
        }

        value = e.getAttribute("gridy");
        if (value.length() == 0)
        {
            this.constraints.gridy = r;
        }
        else
        {
            this.constraints.gridy = Integer.parseInt(value);
        }

        this.constraints.gridwidth = Integer.parseInt(e.getAttribute("gridwidth"));
        this.constraints.gridheight = Integer.parseInt(e.getAttribute("gridheight"));
        this.constraints.weightx = Integer.parseInt(e.getAttribute("weightx"));
        this.constraints.weighty = Integer.parseInt(e.getAttribute("weighty"));
        this.constraints.ipadx = Integer.parseInt(e.getAttribute("ipadx"));
        this.constraints.ipady = Integer.parseInt(e.getAttribute("ipady"));
        ...
        try
        {
            ...
            this.constraints.fill = f.getInt(cl);

            name = e.getAttribute("anchor");
            f = cl.getField(name);
            this.constraints.anchor = f.getInt(cl);
        }
        ...
        Component comp = (Component) this.parseBean((Element) e.getFirstChild());
    ...
    private Object parseBean(Element e)
    {
        try
        {
            ...
            for (int i = 1; i < children.getLength(); i++)
            {
                ...
                Object value = this.parseValue(valueElement);
    ...
    private Object parseValue(Element e)
    {
        ...
        if (child.getTagName().equals("int"))
        {
            ...
            if (child.getTagName().equals("boolean"))
            {
