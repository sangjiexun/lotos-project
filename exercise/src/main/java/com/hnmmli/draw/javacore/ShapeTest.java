...
class ShapeComponent extends JComponent
{
    public ShapeComponent()
    {
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent event)
            {
                ...
                for (int i = 0; i < ShapeComponent.this.points.length; i++)
                {
                    double x = ShapeComponent.this.points[i].getX() - SIZE / 2;
                    double y = ShapeComponent.this.points[i].getY() - SIZE / 2;
                    Rectangle2D r = new Rectangle2D.Double(x, y, SIZE, SIZE);
                    if (r.contains(p))
                    {
                        ShapeComponent.this.current = i;
                        return;
                    }
                }
            }
        ...
        this.addMouseMotionListener(new MouseMotionAdapter() {
            ...
            {
                if (ShapeComponent.this.current == -1)
                {
                    return;
                }
                ShapeComponent.this.points[ShapeComponent.this.current] = event.getPoint();
                ShapeComponent.this.repaint();
            }
        });
        this.current = -1;
    }
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g2.draw(this.shapeMaker.makeShape(this.points));
    }
...
class ArcMaker extends ShapeMaker
{
    ...
    public Shape makeShape(Point2D[] p)
    {
        ...
        double skewedStartAngle = Math.toDegrees(Math.atan2(-(p[2].getY() - centerY) * width, (p[2].getX() - centerX)
                * height));
        double skewedEndAngle = Math.toDegrees(Math.atan2(-(p[3].getY() - centerY) * width, (p[3].getX() - centerX)
                * height));
